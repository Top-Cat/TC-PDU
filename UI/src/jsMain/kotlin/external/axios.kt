...
inline fun <reified T, reified U> generateConfig(ct: CancelToken? = null) = object : AxiosRequestConfig {
    ...
    override var transformResponse: (String) -> U = {
        if (it is U) {
            ...
            pduJson.decodeFromString(it)
        }
    }
    override var validateStatus: ((Number) -> Boolean)? = {
        arrayOf(200, 302).contains(it)
    }
}
...
inline fun <reified T> transformRequest(data: T, headers: dynamic) =
    when {
        ...
        data is String -> data
            ...
            pduJson.encodeToString(data)
        }
    }

inline fun <reified T> axiosGet(url: String) = Axios.get<String>(url, generateConfig<String, String>()).then { pduJson.decodeFromString<T>(it.data) }
