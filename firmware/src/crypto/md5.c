...
void md5_transform (uint32_t[4], uint8_t [64]);
void md5_encode(uint8_t *, uint32_t *, uint32_t);
void md5_decode(uint32_t *, uint8_t *, uint32_t);
...
// Encodes input (uint32_t) into output (uint8_t). Assumes len is a
// multiple of 4.
...
// Decodes input (uint8_t) into output (uint32_t). Assumes len is a
// multiple of 4.
